...
<div class="tuner-tab animated-pane flex flex-col items-center justify-between h-full p-4 md:p-6 w-full max-w-2xl mx-auto">
  <div class="w-full text-center">
    <h2 class="text-xs uppercase tracking-widest text-sky-400 font-bold mb-1">Chromatic Pitch Tuner</h2>
  ...
  <div class="flex flex-col items-center justify-center min-h-[80px] my-2 select-none">
    ...
    {:else}
      <div class="text-xs text-white/30 italic">Tuner Offline. Press START TUNER below.</div>
    {/if}
  </div>
  ...
  <div class="flex items-center gap-3 w-full justify-center">
    ...
    >
      {#if isTuning}
        <MicOff size={15} />
        <span class="text-xs font-bold ml-1.5">STOP TUNER</span>
      {:else}
        <Mic size={15} />
        <span class="text-xs font-bold ml-1.5">START TUNER</span>
      {/if}
    </button>
  </div>
</div>
