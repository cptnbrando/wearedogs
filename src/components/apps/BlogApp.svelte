<script>
  ...
  // Props
  let { initialSlug = $bindable(null), isReading = $bindable(false) } = $props();
...
<svelte:head>
  {#if activePost}
    <title>{activePost.title} | WAD Blog</title>
    ...
    <meta property="og:url" content="{window.location.origin}/apps/blog/{activePost.slug}" />
    <meta property="twitter:title" content={activePost.title} />
    <meta property="twitter:description" content={activePost.description} />
  {:else}
    <title>The Blog | WEAREDOGS</title>
  {/if}
</svelte:head>

<div class="blog-app-container w-full h-full text-white flex flex-col font-sans select-none overflow-hidden relative">
  
  <!-- Master Layout Header -->
  <div class="blog-header shrink-0 px-4 py-3 border-b border-white/5 bg-black/40 backdrop-blur-md flex items-center justify-between z-10">
    <div class="flex items-center gap-2">
      <BookOpen class="text-[#b455ff] w-5 h-5 md:w-6 md:h-6" />
      <span class="font-bold text-sm md:text-base tracking-wider uppercase">DOGS_SYS://READ.LOG</span>
    </div>
    {#if activePost}
      <button 
        type="button"
  ...
  <div class="blog-content-pane flex-grow min-h-0 relative w-full h-full">
    {#if activePost === null}
      ...
      <div class="posts-list-pane w-full h-full overflow-y-auto p-4 md:p-6 flex flex-col gap-3 md:gap-4 max-w-4xl mx-auto">
        <h2 class="text-xs font-bold text-white/40 tracking-widest uppercase mb-1 px-1">LATEST_LOGS_MANIFEST</h2>
        ...
        {:else if loadError && posts.length === 0}
          <div class="border border-red-500/20 bg-red-950/10 p-4 rounded-xl flex items-start gap-2.5">
            <AlertCircle class="text-red-500 shrink-0 w-5 h-5" />
            <div class="text-xs text-red-400 font-mono leading-relaxed">{loadError}</div>
          </div>
        {:else}
            ...
            {#each posts as post}
              ...
              >
                ...
                <div class="flex justify-between items-start gap-2 w-full">
                  <span class="text-[10px] font-mono text-[#b455ff] font-semibold">{post.date}</span>
                  <span class="text-[9px] font-mono text-white/30 tracking-wider">BY: {post.author.toUpperCase()}</span>
                </div>
                
                <!-- Card Title -->
                <h3 class="text-base font-bold leading-snug text-white/90 group-hover:text-white transition-colors">
                  {post.title}
                </h3>
                ...
                <!-- Bottom Indicator -->
                <div class="flex justify-end items-center text-[10px] text-[#b455ff] font-semibold opacity-0 group-hover:opacity-100 transition-opacity gap-1 font-mono pt-1">
    ...
    {:else}
      ...
      <div class="reading-detail-pane w-full h-full overflow-y-auto bg-black/10">
        {#if isLoadingPost}
          <div class="w-full h-full flex flex-col items-center justify-center gap-2">
            <Loader2 class="animate-spin text-[#b455ff] w-10 h-10" />
            <span class="text-xs text-white/40 font-mono">PARSING_MARKDOWN...</span>
          </div>
        {:else if activeContent}
          <article class="w-full max-w-3xl mx-auto px-4 py-6 md:px-8 md:py-10 flex flex-col gap-6 relative select-text">
            ...
            <div class="flex flex-col gap-3 border-b border-white/5 pb-5">
              <div class="flex items-center gap-4 text-xs text-white/40 font-mono">
                <span class="flex items-center gap-1"><Calendar size={12} /> {activePost.date}</span>
                <span class="flex items-center gap-1"><User size={12} /> {activePost.author}</span>
              </div>
              ...
              <p class="text-sm text-white/60 leading-relaxed italic border-l-2 border-[#b455ff]/40 pl-3 py-0.5">
            ...
            <div class="markdown-body text-sm md:text-base leading-relaxed text-white/80 pb-16">
